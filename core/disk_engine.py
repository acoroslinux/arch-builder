...
class DiskEngine:
    ...
    def build_disk_image(self, target_format: str = "img") -> Path:
        ...
        # Ensure target root has autorelabel (mainly for selinux/fedora but safe here)
        ...
        vmlinuz = next((f.name for f in boot_dir.glob("vmlinuz-*") if not f.name.endswith(".old") and "rescue" not in f.name), "vmlinuz-linux")
        initrd = next((f.name for f in boot_dir.glob("initramfs-*.img") if "rescue" not in f.name and "fallback" not in f.name), "initramfs-linux.img")
        ...
        if bootloader_type == "systemd-boot":
            ...
            (loader_dir / "loader.conf").write_text("default arch\\ntimeout 3\\n")
            
            # Create loader/entries/arch.conf
            entries_dir = loader_dir / "entries"
            entries_dir.mkdir(parents=True, exist_ok=True)
            (entries_dir / "arch.conf").write_text(f"""title Arch Linux
linux /{vmlinuz}
initrd /{initrd}
options root=LABEL=ROOTFS rw {kernel_params}
""")
        else:
            if efi_arch_src.exists():
                shutil.copytree(efi_arch_src, self.workdir / "efi_tmp" / "EFI" / "arch", dirs_exist_ok=True)
            ...
            if not bootx64.exists():
                grub = self.workdir / "efi_tmp" / "EFI" / "arch" / "grubx64.efi"
                if grub.exists():
            ...
            grub_cfg = self.workdir / "efi_tmp" / "EFI" / "arch" / "grub.cfg"
