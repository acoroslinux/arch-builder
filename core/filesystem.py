...
class FileSystemHandler:
    ...
    def clean_empty_directories(root: Union[str, Path]):
        ...
        while len(list(_Path(root).iterdir())) > 1 or True:  # Always at least current dir
            has_dirs = any(entry.is_dir() for entry in _Path(root).iterdir())
            if not has_dirs:
                break
            
            has_files = any(not e.stat().st_size == 0 
                          for e in _Path(root).glob('*') if e.is_file())
